...
public class clsPdfWriter_org {
  ...
  //... private Variables
	...
	public clsPdfWriter_org(Shell _ownerShell) {
    ...
    private String BezierCurve(Point ControlPoint1 , Point ControlPoint2 , Point FinalPoint ) {
        ...
        //--Curved path segments are specified as cubic BÃ©zier curves. Such curves are defined by four points: 
    ...
    private String FileHeader(){
        ...
        //-- Note: if (a PDF file contains binary data, as most do (see Section 3.1, â€œLexical Conventionsâ€�),
        //-- it is recommended that the header line be immediately followed by a comment line containing at 
        //-- least four binary charactersâ€”that is, characters whose codes are 128 or greater 
        //-- %Ã¢Ã£Ã�Ã“ these seem to be the standard on files I have tested.
        strFileHeader += "%aaIO" + "\r\n";
        //-- Now give it back
    ...
    private String rootCatalog(){
        ...
        //-- The catalog contains references to other objects defining the documentâ€™s contents,
        ...
        //-- *Required* The page tree node that is the root of the documentâ€™s page tree.
        ...
        //-- *Optional* HideToolbar     - A flag specifying whether to hide the viewer applicationâ€™s tool bars when the document is active. 
        //-- *Optional* DisplayDocTitle - A flag specifying whether the windowâ€™s title bar should display the document title taken from the Title entry
        //-- of the document Information Dictionary
        //-- *Optional* HideWindowUI    - A flag specifying whether to hide user interface elements in the documentâ€™s window (such as scroll bars and navigation controls), leaving only the documentâ€™s contents displayed
   ...
   //... End Region	 
    ...
    private String LoadImgFromJPEGFile(String Name , String FileName ) {
        ...
        for(int i=0; i<6; i+=2){
        	strImage += (char) (bytes[i] << 8 & bytes[i+1]);
        }
        //strImage  +=  "\r\n";
        ...
        try {
        	FileText.append(strImage);
			//strImage += new String(bytes,"ISO-8859-1")+ "\r\n";
		} catch (Exception e) {
    ...
    private  Boolean LawBMP(String FileName , BMPPara p
    	...
    	String bfType  = "OO"; // The string â€œBMâ€� (hex value &H424D).
	  ...
	  //-- Load only the font in use by the application to keep file size small and easy to read.
	    while(keyFonts.hasMoreElements()){
	        ...
	        if (key.startsWith("pdfTrueTypeFonts") ) { //-- Check to see if we need to load any True Type fonts
	            FileText.append(LoadTrueTypeFont(key));
	        }
	    }
	    ...
	    for (String key: keys){
	    	if (colImages.get(key).toString().toUpperCase().endsWith("BMP") == true ) {
	            ...
	            FileText.append(LoadImgFromBMPFile(key, colImages.get(key).toString(), pdfColorSpace.pdfRGB));
	        }
	        ...
	        if (colImages.get(key).toString().toUpperCase().endsWith("JPG") == true || colImages.get(key).toString().toUpperCase().endsWith("JPEG") == true ) {
	        	...
	        	LoadImgFromJPEGFile(key, colImages.get(key).toString());
	        }
        }
